...
if (buildPhi)
    ...
    % Drag objective (Brinkman body force in x1-direction)
    phi = alpha*ux(1);

    %% Integrate and simplify objective functional
...
if (exportPhi)
    fprintf('Exporting phi_drag... \n');
    f = matlabFunction(phi,'File','PHI_drag','Comments','Version: 0.9','Vars',{dx,dy,mu,alpha,transpose([u1 u2 u3 u4 u5 u6 u7 u8 p1 p2 p3 p4])});
    fprintf('Exporting dphi_drag/dg... \n');
    f = matlabFunction(dphidg,'File','dPHIdg_drag','Comments','Version: 0.9','Vars',{dx,dy,mu,alpha,dalpha,transpose([u1 u2 u3 u4 u5 u6 u7 u8 p1 p2 p3 p4])});
    fprintf('Exporting dphi_drag/ds... \n');
    f = matlabFunction(dphids(1:8),'File','dPHIds_drag','Comments','Version: 0.9','Vars',{dx,dy,mu,alpha,transpose([u1 u2 u3 u4 u5 u6 u7 u8 p1 p2 p3 p4])});
end
if (exportJR)
    fprintf('Exporting dr/ds... \n');
    f = matlabFunction(drdg,'File','dRESdg_drag','Comments','Version: 0.9','Vars',{dx,dy,mu,rho,alpha,dalpha,transpose([u1 u2 u3 u4 u5 u6 u7 u8 p1 p2 p3 p4])});
end
%%
time = toc(time);
